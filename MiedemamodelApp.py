...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                             ...
                             QTabWidget, QTextEdit, QFileDialog, QSpinBox, QDoubleSpinBox,
                             QSlider, QCheckBox, QRadioButton, QButtonGroup, QSplitter, QGridLayout, QProgressDialog)
...
class PeriodicTableWidget(QWidget):
	...
	def create_periodic_table (self):
		...
		other_inactive = ["Lv","Fr","Ra","Ac","Rf","Db","Sg","Bh","Hs","Mt","Ds","Rg","Cn","Nh","Fl","Mc"]
...
class ResultsWidget(QWidget):
	...
	def init_ui (self):
		...
		gibbs_label = QLabel("Excess Gibbs Energy (kJ/mol):")
		...
		entropy_label = QLabel("Excess Mixing Entropy (J/mol·K):")
	...
	def set_results (self, enthalpy, gibbs, entropy, composition, details, sub_binaries=None):
		...
		history_entry += f"  Excess Gibbs Energy (kJ/mol): {gibbs:.4f}\n"
		...
		if filename:
			try:
				with open(filename, 'w') as f:
					...
					f.write(f"Excess Gibbs Energy (kJ/mol): {self.gibbs_result.text()}\n")
...
class CompositionVariationWidget(QWidget):
	...
	def calculate_all_properties (self):
		...
		try:
			...
			for model_key in selected_models:
				...
				# 计算过剩吉布斯自由能
				progress.setLabelText(f"计算 {model_key} 模型的过剩吉布斯自由能...")
				valid_compositions = []
				valid_values = []
				...
				for x in comp_range:
					...
					# 计算过剩吉布斯自由能
					try:
						value = UEM.get_excess_Gibbs_byMiedema(
								new_comp, temperature, phase_state, order_degree, model_func)
						
						valid_compositions.append(x)
						valid_values.append(value)
					except Exception as e:
						print(f"计算组成 {new_comp} 的过剩吉布斯自由能时出错: {str(e)}")
						continue
				...
				# 计算过剩熵 (使用ΔH和ΔG计算得到)
				progress.setLabelText(f"计算 {model_key} 模型的过剩熵...")
				valid_compositions = []
				valid_values = []
	...
	def plot_model_comparison (self, model_results, add_element, property_type, matrix_input):
		...
		if property_type == "enthalpy":
			...
			y_label = r"$ΔG^E$ (kJ/mol)"
			title_property = "Excess Gibbs Energy"
		else:  # entropy
			y_label = r"$ΔS^E$ (J/mol·K)"
			title_property = "Excess Entropy"
	...
	def export_to_excel (self, file_path):
		...
		for model in all_models:
			...
			worksheet.write(row, col + 1, f'{model}-过剩吉布斯自由能 (kJ/mol)', header_format)
			worksheet.write(row, col + 2, f'{model}-过剩熵 (J/mol·K)', header_format)
			col += 3
...
class MiedemaModelUI(QMainWindow):
	...
	def calculate (self):
		...
		try:
			# Calculate enthalpy and excess Gibbs energy
			enthalpy = UEM.get_mixingEnthalpy_byMiedema(
					composition, temperature, phase_state, order_degree, model_function)
			
			gibbs = UEM.get_excess_Gibbs_byMiedema(
					composition, temperature, phase_state, order_degree, model_function)
	...
	def get_model_function (self) -> ContributionModelFunc:
		...
		return model_map.get(model_text, UEM.UEM1)
